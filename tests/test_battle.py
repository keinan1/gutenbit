...
# ---------------------------------------------------------------------------
# KEI-230 batch: Johnson & Austen — network tests retained only for
# structurally complex works that are hard to replicate with fixtures.
# The note-heading merge regression is guarded by non-network tests in
# test_html_chunker.py (test_note_heading_not_merged_as_chapter_subtitle,
# test_notes_heading_not_merged_as_subtitle).
# ---------------------------------------------------------------------------


def test_northanger_abbey_separates_note_from_chapter_31():
    """PG 121 — live regression: 'A NOTE ON THE TEXT' stays separate from CHAPTER 31."""
...
def test_lady_susan_nests_letters_and_conclusion_under_title():
    """PG 946 — epistolary: 41 Roman-numeral letters + CONCLUSION nested under title."""
    ...
    letters = [h for h in headings if h.div2 and h.div2 != "CONCLUSION"]
...
def test_love_and_freindship_nests_works_under_collection_titles():
    """PG 1212 — multi-work collection: letters under LOVE AND FREINDSHIP, A TALE standalone."""
...
def test_preface_to_shakespeare_keeps_selected_notes_sections():
    """PG 5429 — mixed nesting: essay section + play-keyed notes at varying depths."""
...
def test_rambler_vol1_nests_essays_under_contents():
    """PG 43656 — 105 'No. N.' essays nested under CONTENTS at scale."""
